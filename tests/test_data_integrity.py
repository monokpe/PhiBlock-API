...
class TestTokenUsageIntegrity:
    ...
    def test_token_usage_aggregation(self, db_session, test_tenant):
        ...
        total = (
            db_session.query(func.sum(TokenUsage.total_tokens))
            .filter_by(tenant_id=test_tenant["tenant"].id)
            .scalar()
        )
...
class TestDataRetention:
    ...
    def test_retention_policy_respects_tenant(self, db_session, test_tenant):
        ...
        db_session.query(AuditLog).filter_by(tenant_id=test_tenant["tenant"].id).delete()
        db_session.commit()
